    ...
    try {
      ...
      for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
        ...
        for (const toolCall of toolCalls) {
          ...
          if (!providerTool) {
            throw new ProviderError(`Tool ${toolName} not found in request`, 'TOOL_NOT_FOUND')
          }
          ...
          const result = await executeTool(providerTool as any, preparedExecution.params, {
            workflowId: request.workflowId,
            workspaceId: request.workspaceId,
            userId: undefined,
          ...
          trackForcedToolUsage(preparedTools, toolName, usedForcedTools)
      ...
      throw new ProviderError('Maximum tool iterations reached', 'MAX_TOOL_ITERATIONS')
    } catch (error) {
      ...
      throw new ProviderError(toError(error).message, 'PROVIDER_ERROR')
    }
  },
}
