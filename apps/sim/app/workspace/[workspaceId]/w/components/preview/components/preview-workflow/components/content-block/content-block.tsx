...
export const PreviewContentBlock = memo(function PreviewContentBlock({
	...
	const fontSize = extractStoredValue<number>(
		data.subBlockValues,
		"fontSize",
		16,
	);
	...
	const file = extractStoredValue<UploadedFileValue | null>(
		data.subBlockValues,
		"file",
		null,
	);
	...
	return (
		<div
			className={cn(
				...
				!data.isPreviewSelected &&
					hasSuccess &&
					"ring-[1.75px] ring-[var(--brand-accent)]",
				!data.isPreviewSelected &&
					hasError &&
					"ring-[1.75px] ring-[var(--text-error)]",
			)}
			style={
				variant === "image"
