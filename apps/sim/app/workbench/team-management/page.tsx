...
export default function TeamManagementPage() {
  ...
  const { data: workgroupData, isLoading } = useMyWorkgroups(Boolean(session?.user))
  ...
  const activeWorkgroupId = selectedWorkgroupId ?? workgroupData?.defaultWorkgroupId ?? workgroupData?.workgroups[0]?.id
  ...
  const { data: membersData } = useWorkgroupMembers(activeWorkgroup?.role === 'admin' ? activeWorkgroup.id : undefined)
  ...
  if (isPending || isLoading) {
    return <div className='flex min-h-screen items-center justify-center text-muted-foreground'>加载团队管理...</div>
  }
    ...
    return (
      <main className='min-h-screen bg-[#f7f4ed] px-6 py-10'>
        <section className='mx-auto max-w-3xl rounded-[2rem] border border-[#e2d8c7] bg-white p-8 shadow-sm'>
          <Link href='/workbench' className='text-sm font-semibold text-[#9b5b2e]'>返回工作台</Link>
          <h1 className='mt-4 text-3xl font-semibold tracking-tight text-[#271f18]'>需要团队管理员权限</h1>
    ...
    <main className='min-h-screen bg-[#f7f4ed] px-6 py-8 text-[#271f18]'>
      <section className='mx-auto max-w-6xl space-y-5'>
        <div className='rounded-[2rem] border border-[#e2d8c7] bg-white p-6 shadow-sm'>
          ...
          <div className='mt-4 flex flex-col gap-4 md:flex-row md:items-end md:justify-between'>
            <div>
              ...
              <p className='mt-3 text-[#6f6256]'>管理当前工种团队成员。加入成员后会同步团队画布 workspace 权限。</p>
            </div>
            <select
        ...
        >
          ...
          <button className='rounded-xl bg-[#271f18] px-5 py-3 text-sm font-semibold text-white' type='submit'>添加成员</button>
        </form>
        ...
        <div className='overflow-hidden rounded-[1.5rem] border border-[#e2d8c7] bg-white shadow-sm'>
          ...
          {(membersData?.members ?? []).map((member) => (
            <div key={member.userId} className='grid grid-cols-[1.2fr_1fr_160px_180px] items-center border-b border-[#f2eadf] px-5 py-4 text-sm last:border-b-0'>
              ...
              <button
                ...
                type='button'
                onClick={() => removeMember.mutate({ workgroupId: activeWorkgroup.id, userId: member.userId })}
              >
                移除
              </button>
            </div>
