...
const { mockDbSelect, mockIsOrganizationOnEnterprisePlan } = vi.hoisted(() => ({
  mockDbSelect: vi.fn(),
...
describe('GET /api/organizations/[id]/data-retention', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns data retention settings for organization members', async () => {
    ...
    expect(data.data.isEnterprise).toBe(true)
    expect(data.data.configured).toEqual({
      logRetentionHours: 240,
      softDeleteRetentionHours: 480,
      taskCleanupHours: 72,
    })
