...
import { trimWorkspaceVideoContract } from '@/lib/api/contracts/media-videos'
import { parseRequest } from '@/lib/api/server'
import { checkSessionOrInternalAuth } from '@/lib/auth/hybrid'
import { withRouteHandler } from '@/lib/core/utils/with-route-handler'
import { trimWorkspaceVideo } from '@/lib/generated-media/video/video-trim-service'
...
const logger = createLogger('MediaVideosTrimAPI')
...
export const POST = withRouteHandler(async (request: NextRequest) => {
  ...
  const validation = await parseRequest(trimWorkspaceVideoContract, request, {}, {})
  if (!validation.success) return validation.response

  const { workspaceId, sourceFile, startSeconds, endSeconds } = validation.data.body
  ...
  try {
    ...
    logger.error('Video trim failed', { workspaceId, userId: auth.userId, error: message })
