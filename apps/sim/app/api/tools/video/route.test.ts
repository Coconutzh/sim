...
vi.mock('@/lib/api/contracts/tools/media/video', () => ({
  videoProviders: ['runway', 'veo', 'luma', 'minimax', 'falai'],
  videoToolContract: {},
}))
...
vi.mock('@/lib/core/execution-limits', () => ({
  getMaxExecutionTimeout: vi.fn(() => 60_000),
}))

vi.mock('@/lib/core/config/env', () => ({
  env: {
    RUNWAY_API_KEY: 'server-key',
  },
  getEnv: vi.fn(() => ''),
...
describe('tools video route', () => {
  ...
  it('hides foreign personal execution workspaces behind 404', async () => {
    ...
    const response = await POST(
      new NextRequest('http://localhost/api/tools/video', {
        method: 'POST',
