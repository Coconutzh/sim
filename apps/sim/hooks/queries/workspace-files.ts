...
export function useWorkspaceFileContent(
	...
	return useQuery({
		queryKey: [
			...workspaceFilesKeys.content(workspaceId, fileId, raw ? "raw" : "text"),
			key,
		],
...
export function useWorkspaceFileBinary(
	...
	return useQuery({
		queryKey: [
			...workspaceFilesKeys.content(workspaceId, fileId, "binary"),
			key,
		],
...
async function uploadViaApiFallback(
	...
	// boundary-raw-fetch: local fallback uploads stream the raw file body because
	// large media files can fail Undici's multipart FormData parsing in dev.
	const response = await fetch(`/api/workspaces/${workspaceId}/files`, {
		method: "POST",
		headers: {
			"Content-Type": file.type || "application/octet-stream",
			"X-Upload-File-Name": encodeURIComponent(file.name || "untitled"),
...
async function parseUploadResponse(
	...
	let data: { success?: boolean; error?: string; file?: UserFile } | null =
		null;
	try {
...
async function uploadWorkspaceFile(
	...
	let result: UploadStrategyResult;
	try {
...
export function useUploadWorkspaceFile() {
	...
	return useMutation({
		...
		onSettled: (_data, _error, _variables) => {
			if (_variables.skipInvalidation) return;
		...
		onError: (error, variables) => {
			...
			if (!variables.skipToast) {
				toast.error(
					`Failed to upload "${variables.file.name}": ${error.message}`,
					{
						duration: 5000,
					},
				);
...
export function useUpdateWorkspaceFileContent() {
	...
	return useMutation({
		...
		onSettled: (_data, _error, variables) => {
			queryClient.invalidateQueries({
				queryKey: workspaceFilesKeys.contentFile(
