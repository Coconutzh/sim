...
interface BunBuildError {
  errors?: BunBuildMessage[]
}
...
const isRecoverableWindowsBundleReadError = (error: unknown): error is BunBuildError => {
  if (process.platform !== 'win32' || !error || typeof error !== 'object') {
    return false
  }

  const buildMessages = (error as BunBuildError).errors
  if (!Array.isArray(buildMessages) || buildMessages.length === 0) {
    return false
  }

  return buildMessages.some((entry) => {
    if (typeof entry?.message !== 'string') {
      return false
    }

    return entry.message.includes('EPERM') && entry.message.includes('node_modules')
  })
}

const getBundleErrorMessages = (error: BunBuildError): string[] =>
...
async function main(): Promise<void> {
  ...
  for (const spec of BUNDLES) {
    ...
    const outPath = join(BUNDLES_DIR, spec.outFile)
    ...
    try {
      ...
      if (existsSync(outPath) && isRecoverableWindowsBundleReadError(error)) {
        logger.warn(`reusing checked-in sandbox bundle for ${spec.name}`, {
          reasons: getBundleErrorMessages(error),
        })
        continue
      }
