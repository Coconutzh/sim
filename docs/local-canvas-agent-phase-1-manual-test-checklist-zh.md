...
| B-03 | 当前源码服务级通过；读取完整 selected video detail，file 只显示文件名，无 key/path/url。 | 浏览器确认视频节点展示和脱敏输出。 |
...
| E-03 | current-source preview API/state 证据通过：一次性 workflow `1567e11e-b68b-46a6-b7e1-2d7a04598f5f` 中选中 `e03-video-node`，初始 `videoParameters={"resolution":"720P","duration":8}`；发送“把视频时长改成 5 秒，并让镜头更有推进感。”后，SSE 包含 apply/verify，UTF-8 原始 state 复核显示 `videoPrompt` 追加“5 秒，并让镜头更有推进感。”，`videoParameters={"resolution":"720P","duration":5}`。 | 仍可补强右侧属性面板浏览器展示；字段写入和 verify 已有 current-source preview API/state 证据。 |
| E-04 | current-source preview API/state 证据通过：同一 workflow 中选中 `e04-audio-node`，初始 `audioPrompt=基础配乐方向。` 且 `file=null`；发送“把音乐方向改成更有节奏感的电子风格。”后，SSE 包含 apply/verify，不包含 generate，UTF-8 原始 state 复核显示 `audioPrompt` 追加“方向更有节奏感的电子风格。”，`file` 仍为 null。 | 仍可补强右侧属性面板浏览器展示；字段写入和 verify 已有 current-source preview API/state 证据。 |
| F-01 | 当前源码服务级通过；节点位置实际变化，节点和边未丢失。2026-06-07 20:35 current-source 浏览器复测继续复现 UI 缺口：真实页面发送布局请求后，assistant 显示“已完成画布修改，并完成验证”，workflow state 从凌乱 position 改为横向顺序，但 ReactFlow DOM transform 仍停留在旧凌乱位置。本轮已修改 `copilot-tab.tsx`，让 local canvas mutation tool result 和 stream end reload committed workflow state；定向 Biome 与 `apps/sim` type-check 通过。追加 stream-end 兜底后的浏览器复测被 Next dev server OOM / ConnectionRefused 打断，尚无通过证据。 | 必须重启稳定 current-source dev server 后继续确认：不刷新页面时 ReactFlow DOM transform 同步变为新 position。 |
