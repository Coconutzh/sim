...
| F-01 | 位置同步已有 current-source 浏览器证据和 focused 自动回归；2026-06-08 本轮追查又发现旧样本 workflow 中 content-reference edge 只有 `content-reference-*` handles、但缺少 `data.kind=content_reference`，刷新后可能节点 position 对齐但连接不显示。已提交 `8a8e8e38c` 在 `normalizeWorkflowState()` 中兼容升级旧 edge，并提交 `7e31e4c58` 修复 `ContentBlock` 缺失 `coerceNumber` 导致的当前源码页面 error boundary。 | F-01 不再是纯代码阻塞；但本轮 3006 current-source dev server 仍停在按需编译 / workspace shell loading，尚未补到修复后的同端口 ReactFlow DOM `nodeCount=7 + edgeCount=5` 新证据。下次稳定 current-source 页面挂载后需优先复测连接显示。 |
