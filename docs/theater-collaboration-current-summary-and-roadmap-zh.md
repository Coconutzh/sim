...
- `bun run check:api-validation:strict` 当前基线为 `total=761, zod=736, nonZod=25`，最近 shared contract ID errors 切片未新增 route 或改变 boundary policy 计数；严格校验继续通过。
- `bun run type-check` 仍退出 2，但按最新 shared contract ID errors 切片触碰路径和文案标识过滤输出 `NO_TOUCHED_PATH_TYPECHECK_MATCHES`；全量 type-check 仍有仓库既有历史错误，不能宣称全量通过。
