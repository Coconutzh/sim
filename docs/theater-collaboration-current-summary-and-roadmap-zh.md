...
```powershell
Set-Location apps\sim; bunx biome check --write "lib/billing/billing-lifecycle-audit.ts" "lib/billing/webhooks/subscription.ts" "lib/billing/webhooks/subscription.test.ts" "lib/collaboration/service.ts" "lib/collaboration/service.test.ts"
Set-Location apps\sim; bunx biome check "lib/billing/billing-lifecycle-audit.ts" "lib/billing/webhooks/subscription.ts" "lib/billing/webhooks/subscription.test.ts" "lib/collaboration/service.ts" "lib/collaboration/service.test.ts"
Set-Location apps\sim; bunx vitest run lib/billing/webhooks/subscription.test.ts lib/collaboration/service.test.ts
bun run check:api-validation:strict
$patterns = @('organization.subscription_cancelled','subscription_cancelled','handleSubscriptionDeleted','billing-lifecycle-audit','OrganizationBillingLifecycle','recordOrganizationBillingLifecycleAudit','billing/webhooks/subscription','billing_management','ProjectNotificationCenter','projectNotification','notifications/center','lib/collaboration/service'); $output = bun run type-check 2>&1; $matches = $output | Select-String -Pattern $patterns; if ($matches) { $matches | ForEach-Object { $_.Line }; exit 1 } else { 'NO_TOUCHED_PATH_TYPECHECK_MATCHES' }
git diff --check
```
...
- `bun run type-check` 仍退出 2，但按本轮触碰路径（含 `organization.subscription_cancelled`、subscription_cancelled、handleSubscriptionDeleted、`billing-lifecycle-audit`、OrganizationBillingLifecycle、recordOrganizationBillingLifecycleAudit、`billing/webhooks/subscription`、`billing_management`、project notification center 相关符号、`notifications/center`、collaboration service）过滤输出 `NO_TOUCHED_PATH_TYPECHECK_MATCHES`；全量 type-check 仍有仓库既有历史错误，不能宣称全量通过。
