...
### 5.23 Phase 10 项目级创建团队切片

本轮继续推进项目管理员中心，从只读概览增加第一个受控写操作：

- `ProjectAdminCenter` 新增 `Create project team` 表单，项目管理员可输入团队名并选择工种。
- 创建动作复用现有 `useCreateWorkgroup` hook、`POST /api/organizations/[id]/workgroups` contract 和 `createWorkgroup` 服务；后端仍由 `assertOrganizationAdmin` 保护，不新增 route-local schema 或新的权限边界。
- 创建成功后既有 hook 会刷新组织团队列表和我的团队列表；服务端继续自动创建 workgroup、team canvas workspace、workspace admin permission 和默认 `Team canvas` workflow graph。
- 表单保持在原 `/workspace/[workspaceId]/project-admin` shell 内，沿用 `var(--surface-*)`、`var(--border)`、`buttonVariants` 和 8px 圆角，不引入全局样式。

仍需继续：

- Phase 10 仍缺团队归档、设置团队管理员、项目级成员批量分配、Agent 模板策略、全局审计筛选和完整状态树治理；创建团队只是项目级管理写操作的第一步。

### 5.24 Phase 10 项目级成员分配切片

本轮继续把项目管理员中心从项目级建队推进到项目级人员分配：

- `ProjectAdminCenter` 新增 `Assign project member` 表单，项目管理员可输入既有用户 email 或 user ID，选择目标团队，并指定 `member` 或 `admin` 角色。
- 分配动作复用现有 `useAddWorkgroupMember` hook、`POST /api/workgroups/[workgroupId]/members` contract 和 `addWorkgroupMember` 服务；后端仍走 `assertWorkgroupAdmin`，组织 owner/admin 可跨团队执行，团队管理员仍只管理自己的团队。
- `useAddWorkgroupMember` 增加可选 `organizationId` 变量；项目管理员中心传入后会同时刷新组织团队列表，确保成员数量在操作后回到最新状态。
- 表单继续保持在原 `/workspace/[workspaceId]/project-admin` shell 内，不新增 API route，不新增 route-local schema，不引入全局样式。
...
本轮继续推进项目管理员中心的治理可见性，把团队级 activity 能力接入项目级入口：

- `ProjectAdminCenter` 新增 `Project activity drilldown` 区块，项目管理员可在项目中心选择任意团队查看最近 activity。
- 该区块复用既有 `useWorkgroupActivity` hook 和 `GET /api/workgroups/[workgroupId]/activity` contract，不新增 route-local schema，也不新增项目级专用 audit API。
- 后端仍由 `assertWorkgroupAdmin` 保护；组织 owner/admin 可通过既有 org admin 语义查看任意团队 activity，团队管理员仍只可查看自己团队。
- 该切片让 Phase 10 先具备“按团队 drilldown”的审计可见性，但仍不是完整的组织级全局审计筛选器。
...
- 当前建议只是 access-map based 首版，不理解人员工种、默认团队归属、seat 余量或历史项目偏好；后续 5.29 已补本地文件导入首版，Phase 10 仍需团队归档/恢复、全局审计筛选、项目级 Agent 模板策略和完整状态树治理。

### 5.29 Phase 10 项目级成员文件导入切片

本轮继续补齐项目管理员批量分配的输入来源，保持“前端导入、显式提交、逐项复用现有 API”的边界：

- `ProjectAdminCenter` 在 `Batch assign` 区块新增 `Import from file`，支持 CSV、TSV、TXT 本地文件。
- 导入逻辑逐行解析文件，优先取包含 `@` 的 email 单元格；如果该行没有 email，则取第一个非 header/role/team 语义的单元格作为 user ID 候选。
- 导入结果会合并进现有 textarea，并继续复用 `parseBatchAssignmentTargets` 做大小写不敏感去重；不会自动写入成员关系。
- 写入仍必须点击 `Assign batch`，继续逐项调用 `useAddWorkgroupMember` / `POST /api/workgroups/[workgroupId]/members`，没有新增 bulk route、contract 或权限边界。
